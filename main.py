    ...
    def createChart(self, title, days, type, db):
        ...
        if(type == "lines"):
            ...
            for name in names:
                ...
                series.append(self.fillLineSeries(newSeries, datesAvailable, db, title))
            ...
            for name in names:
                ...
                series.append(self.fillBarSeries(newSeries, datesAvailable, db, title))
            ...
            for name in names:
                ...
                series.append(self.fillScheduleSeries(newSeries, datesAvailable, db, title))
...
# TODO : Los axis sean fechas de la bd, no generadas.
    ...
    def fillLineSeries(self, series, dateseries, db, title):
        ...
        print(title)
        records = db.find({ "date": { '$gte': dateseries[0], '$lte': dateseries[-1]}, title : {"$exists": True}},{title : 1, '_id' : False})
    ...
    def fillScheduleSeries(self, series, dateseries, db, title):
        ...
        records = db.find({ "date": { '$gte': dateseries[0], '$lt': dateseries[-1]},title : {"$exists": True}},{title : 1, '_id' : False}).sort("date", -1).limit(1)
